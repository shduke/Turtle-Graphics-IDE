...
public class FunctionAssignmentBehavior extends UnaryBehavior<String, IVariable> {
    ...
    public FunctionAssignmentBehavior(Function<String, IVariable> operation, Map<String, IVariable> variableMap, AbstractCommand... arguments) {
        super(operation, arguments);
        myParameters = arguments[EXPRESSION_PARAMETER_INDEX].getCommandArguments().stream().map(p->(new Variable(variableMap::put, p.toString()))).toArray(IVariable[]::new);
        Arrays.asList(myParameters).stream().forEach(p -> p.execute());
        //arguments[EXPRESSION_PARAMETER_INDEX].getCommandArguments().stream().map(p->{}).forEach(p->(new Set(operation, arguments)).execute())
    }
